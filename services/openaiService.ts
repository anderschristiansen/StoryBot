...
Theme: ${theme.title} | Age-appropriate for ${protagonist.age}-year-olds`;
  ...
  static async generateStory({ theme, protagonist }: StoryGenerationRequest): Promise<Story> {
    try {
...
OPGAVE: Skriv engagerende, interaktive børnehistorier på fejlfrit dansk med meningsfulde valg, der påvirker historiens udvikling.`
      ...
      // Validate Danish quality in the story
      await this.validateDanishQuality(storyData, protagonist);
  ...
  private static async validateDanishQuality(storyData: any, protagonist: UserProfile): Promise<void> {
    try {
...
Er sproget korrekt dansk? List eventuelle fejl.`
  ...
  static async generateImage(prompt: string): Promise<string> {
    try {
      ...
      return 'https://via.placeholder.com/1024x1024/FFB6C1/000000?text=Billede+ikke+tilgængeligt';
    }
  }
